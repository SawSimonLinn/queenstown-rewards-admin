...
export default async function StaffPage() {
  ...
  const { data: staffMembers } = await supabase
    .from("staff_members")
    .select("id, is_active, profiles(id, full_name, email, role), locations(name)")
    .order("id");
  const staffItems = [...(staffMembers ?? [])].sort(
          ...
          {staffItems.map((member, index) => {
            ...
            return (
              ...
              >
                ...
                <form
                  action={setStaffActive.bind(null, member.id, !member.is_active)}
                  className="mt-4"
                >
                  <SubmitButton
                    variant="outline"
